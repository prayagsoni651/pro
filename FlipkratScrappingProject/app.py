...
class threadClass:
    ...
    def run(self):
        ...
        free_status = False
        # Get credentials from env or use defaults (but better to use env)
        username = os.getenv("MONGO_USERNAME")
        password = os.getenv("MONGO_PASSWORD")
        uri = os.getenv("MONGO_URI")
        
        # Note: Scrapper object's getReviewsToDisplay method needs to accept these or be refactored
        # The current implementation passes username/password to getReviewsToDisplay.
        # We need to make sure we pass the correct ones.
...
def index():
    if request.method == 'POST':
        ...
        try:
            ...
            if uri:
                 mongoClient = MongoDBManagement(uri=uri)
            else:
